 ...
 * tapping it again opens the leg detail.
 */
@Composable
fun PlanningPage(
    ...
    Surface(
      ...
      Row(verticalAlignment = Alignment.CenterVertically) {
        Column(Modifier.weight(1f).padding(start = 16.dp, top = 6.dp, bottom = 6.dp)) {
          EndpointField(
            label = stringResource(R.string.planning_from),
            value = from?.let {
          ...
          EndpointField(
            label = stringResource(R.string.planning_to),
            value = to?.let {
              if (it.isUserLocation) stringResource(R.string.planning_my_location) else it.name
            },
          ...
          LazyColumn(contentPadding = androidx.compose.foundation.layout.PaddingValues(
            ...
            items(s.itineraries) { itinerary ->
              ItineraryCard(
                ...
                onClick = {
                  if (itinerary === selected) onOpenDetail() else viewModel.select(itinerary)
                },
              )
            }
          }
